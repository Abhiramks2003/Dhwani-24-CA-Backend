...
const serviceAccountKeyFile = "service-account-credentials.json";
const sheetId = "1123ayrIlZoABp80Fhv2wHLHtgxsYEaM5ErPM71T5Wjk";
...
app.get("/", async (req, res) => {
  try {
    const url1 =
      "https://dev.bharatversity.com/events/website/api/hosted-events-api/?limit=100&offset=00";
        ...
        for (let j = 0; j < participants.length; j++) {
          try {
            if (participants[j]["extra_questions"].length !== 0) {
              const referal_code =
                participants[j]["extra_questions"][0]["answer"];
              console.log(referal_code);

              for (let k = 0; k < data.length; k++) {
                if (
                  data[k].referal_code.toLowerCase().trim() ===
                  referal_code.toLowerCase().trim()
                ) {
                  let points = 0;
...
app.get("/campus-ambassadors", async (req, res) => {
  try {
    const googleSheetClient = await _getGoogleSheetClient();
    let data = await _readGoogleSheet(
      googleSheetClient,
      sheetId,
      tabName,
      range,
